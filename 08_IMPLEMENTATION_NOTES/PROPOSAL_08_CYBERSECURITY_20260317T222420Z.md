...
This script defines a report generator for red team campaigns, adhering to the specified requirements. It reads `ProbeResults` from a JSON file, processes the data, and generates a Markdown report with the required sections. The script ensures that the output is valid Markdown and includes all necessary metadata and findings.